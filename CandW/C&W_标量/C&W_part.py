...
if __name__ == "__main__" :
 ...
 [-0.28298028]]
    ...
    # print(U)
    # print( b )
    delta_W = np.mat(np.zeros([hiden_num, m * windows]))
    delta_U = np.mat(np.zeros([hiden_num, 1]))
    delta_b = np.mat(np.zeros([hiden_num, 1]))
        ...
        if len(sentence) >= windows :
            ...
            for i in range( len(sentence) - windows + 1 ) :
                ...
                if max( 0 , 1 - f + f_rand_sample ) > 0 :
                    ...
                    delta_U = np.mat(np.zeros([hiden_num, 1]))
                    delta_b = np.mat(np.zeros([hiden_num, 1]))
                    delta_x = np.mat(np.zeros([1,m * windows])) #将x置0
                    delta_x_rand_sample = np.mat(np.zeros([1, m * windows]))  # 将x置0
                    for q in range( hiden_num ) :
                        delta_U[ q ] = h_rand_sample[q] - h[ q ]
                        if (l[q] < 1 and -1 < l[q]) and (l_rand_sample[q] >= 1 or -1 >= l_rand_sample[q]) :
                            delta_b[q] = -U[q]
                            for j in range(m * windows):
                                delta_x[0, j] -= U[q] * W[q, j]
                                delta_W[q, j] = - x[0, j] * U[q]
                        elif (l[q] >= 1 or -1 >= l[q]) and (l_rand_sample[q] < 1 and -1 < l_rand_sample[q]):
                            delta_b[q] = U[q]
                            for j in range(m * windows):
                                delta_x_rand_sample[0, j] += U[q] * W[q, j]
                                delta_W[q, j] = U[q] * x_rand_sample[0, j]
                        elif (l[q] < 1 and -1 < l[q]) and (l_rand_sample[q] < 1 and -1 < l_rand_sample[q]):
                            for j in range(m * windows):
                                delta_x[0, j] -= U[q] * W[q, j]
                                delta_x_rand_sample[0, j] += U[q] * W[q, j]
                                delta_W[q, j] = U[q] * (x_rand_sample[0, j] - x[0, j])
